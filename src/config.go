...
func (element *ConfigST) AssignUrl() {
	element.mutex.Lock()
	defer element.mutex.Unlock()
	if tmp, ok := element.Streams["wide"]; ok {
		if !tmp.RunLock {
			tmp.RunLock = true
			//tmp.URL = "rtsp://admin:Getac10203A@192.168.5.49:554/live/0"
			tmp.URL = fmt.Sprintf("rtsp://%s:%s@%s:554/live/0", Account.user, Account.pwd, BoardInfo.IPv4)
			Log.Infof(">>>>[%s]", tmp.URL)
			element.Streams["wide"] = tmp
			tmp.RunLock = false
		}
	}

	if tmp, ok := element.Streams["narrow"]; ok {
		if !tmp.RunLock {
			tmp.RunLock = true
			//tmp.URL = "rtsp://admin:Getac10203A@192.168.5.49:554/live/1"
			tmp.URL = fmt.Sprintf("rtsp://%s:%s@%s:554/live/1", Account.user, Account.pwd, BoardInfo.IPv4)
			Log.Infof(">>>>[%s]", tmp.URL)
			element.Streams["narrow"] = tmp
			tmp.RunLock = false
		}
	}
}
